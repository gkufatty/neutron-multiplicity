...
TPC determineTPC(const caf::SRVector3D & pos) {
    ...
    // Check if Y is out of global bounds
    if (y < -constants::Y_BOUND || y > constants::Y_BOUND) {
        return TPC::Outside;
    }

    for (const auto& mapping : tpc_map) {
        if (x >= mapping.x_bound[0] && x <= mapping.x_bound[1] &&
            y >= mapping.y_bound[0] && y <= mapping.y_bound[1] &&
            z >= mapping.z_bound[0] && z <= mapping.z_bound[1]) {
            return mapping.tpc;
        }
    }
...
std::string tpcToString(TPC tpc) {
    switch(tpc) {
        case TPC::TPC0: return "TPC0 (Module 0, IO Group 1, Right Top)";
        case TPC::TPC1: return "TPC1 (Module 0, IO Group 2, Right Top)";
        case TPC::TPC2: return "TPC2 (Module 1, IO Group 3, Right Bottom)";
        case TPC::TPC3: return "TPC3 (Module 1, IO Group 4, Right Bottom)";
        case TPC::TPC4: return "TPC4 (Module 2, IO Group 5, Left Top)";
        case TPC::TPC5: return "TPC5 (Module 2, IO Group 6, Left Top)";
        case TPC::TPC6: return "TPC6 (Module 3, IO Group 7, Left Bottom)";
        case TPC::TPC7: return "TPC7 (Module 3, IO Group 8, Left Bottom)";
        case TPC::Outside: return "Outside all TPCs";
        default: throw std::runtime_error("Unknown TPC");
    }
}
